...
### Added
- **Scope Selection in Skill Installation**: Added a scope selection prompt to let the user select between User level (Global, home directory) and Workspace level (Local project) when running `issuer skill install`.
...
### Changed
- **Interactive Skill Installation Sorting**: Re-ordered the interactive selection list in `issuer skill install` to place Universal Agents, Claude Code, and Cursor consistently in the top three positions.
...
### Added
- **Gemini CLI Support**: Added path detection for Gemini CLI in `issuer skill install` to support dynamic user-level skill installation inside `~/.gemini/skills/`.
